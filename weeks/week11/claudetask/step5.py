...
S1 = Si["S1"]
ST = Si["ST"]
week6_standard_errors = np.array([
    0.12800909,   # CL
    71.26673151,  # Q
    1.12327614,   # V1
    3.44752081    # V2
...
week6_parameter_names = ["CL", "Q", "V1", "V2"]
week10_parameter_names = problem["names"]
week6_units = ["L/h", "L/h", "L", "L"]
week10_units = ["L/h", "L/h", "L", "L"]
df_sensitivity["S1 Rank"] = pd.Series(S1).rank(ascending=False)
df_sensitivity["ST Rank"] = pd.Series(ST).rank(ascending=False)
df_sensitivity["Standard Error"] = pd.Series(week6_standard_errors)
