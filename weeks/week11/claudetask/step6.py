...
Si = sobol_analyze.analyze(problem, auc_results, calc_second_order=True, print_to_console=False)
S1 = Si["S1"]
ST = Si["ST"]
...
df_sensitivity["S1 Rank"] = pd.Series(S1).rank(ascending=False)
df_sensitivity["ST Rank"] = pd.Series(ST).rank(ascending=False)
df_sorted = df_sensitivity.sort_values(by=["ST"], ascending=False)
x = np.arange(len(df_sorted))
width = 0.35

plt.bar(x - width/2, df_sorted["S1"], width, label="S1")
plt.bar(x + width/2, df_sorted["ST"], width, label="ST")

plt.xticks(x, df_sorted["Parameter"])
plt.xlabel("Parameter")
plt.ylabel("Sobol Sensitivity Index (dimensionless)")
plt.title("First-Order vs Total-Order Sobol Sensitivity")
plt.legend()
plt.tight_layout()
plt.show()
