...
#--------
# 5.2 OBJECTIVE CONTOUR PLOT
#--------

D_grid, tau_grid = np.meshgrid(D_values, tau_values)

objective_grid = np.zeros_like(D_grid)

for i in range(len(tau_values)):
    for j in range(len(D_values)):
        auc_24 = regimen_auc24(
            D_grid[i, j],
            tau_grid[i, j],
            default_params.CL
        )

        objective_grid[i, j] = objective(
            auc_24,
            auc_target
        )
...
plt.colorbar(label="Objective |AUC24 - AUC target|")
plt.xlabel("Dose D (mg)")
plt.ylabel("Dosing Interval τ (hours)")
plt.title("Objective Function over Dose and Dosing Interval")
plt.show()