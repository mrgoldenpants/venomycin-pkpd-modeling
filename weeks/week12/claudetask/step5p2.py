...
start_1 = [600, 20]
start_2 = [1800, 8]
def regimen_objective(x):
    D, tau = x
    auc_24 = regimen_auc24(D, tau, default_params.CL)
    return objective(auc_24, auc_target)

result_1 = minimize(
    regimen_objective,
    start_1,
    bounds = [
    (default_params.Dmin, default_params.Dmax),
    (default_params.tau_min, default_params.tau_max)
    ]
)
...
D_opt, tau_opt = result_1.x

auc_opt = regimen_auc24(D_opt, tau_opt, default_params.CL)
auc_mic_opt = calculate_auc_mic(auc_opt, default_params.MIC)
...
def simulate_repeated_regimen(dose, tau, params, total_time=48):
    y_current = np.array([0.0, 0.0])
    all_times = []
    all_concentrations = []
    dose_times = np.arange(0, total_time, tau)
    ...
    auc = np.trapezoid(all_concentrations, all_times)
    return {
        "time": all_times,
...
validation = simulate_repeated_regimen(
        D_opt,
        tau_opt,
        default_params
)

validation_auc_mic = calculate_auc_mic(validation["auc"], default_params.MIC)

assert(default_params.AUC_MIC_lower <= validation_auc_mic <= default_params.AUC_MIC_upper)
