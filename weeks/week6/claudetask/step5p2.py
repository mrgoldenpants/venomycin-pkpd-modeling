import pandas as pd
import numpy as np
from scipy.integrate import solve_ivp
from scipy.optimize import least_squares
initial_guess = [4, 4, 4, 4]
df = pd.read_csv("pk_dataset_3.csv")
lower_bounds = [0.05, 0.01, 3, 3]
upper_bounds = [15, 15, 20, 30]
def residuals(theta, observed_time, observed_concentration):
    CL, Q, V1, V2 = theta
    y0 = np.array([100, 0])

    def two_compartment_ode(t, y, CL, Q, V1, V2):
        A1, A2 = y
        dA1_dt = -CL / V1 * A1 - Q * A1 / V1 + Q * A2 / V2
        dA2_dt = Q * A1 / V1 - Q * A2 / V2
        return np.array([dA1_dt, dA2_dt])

    predicted_solution = solve_ivp(
        two_compartment_ode,
        [0, 48],
        y0,
        args=(CL, Q, V1, V2),
        t_eval=observed_time
    )

    A1 = predicted_solution.y[0]
    predicted_concentration = A1 / V1
    return observed_concentration - predicted_concentration
def fit_model(data):
    fitted_parameters = []
    subjects = data["Subjects"].unique()
    for subject in subjects:
        subject_data = data[data["Subjects"] == subject]
        observed_time = subject_data["Time"].values
        observed_concentration = subject_data["C"].values
        result = least_squares(residuals, initial_guess, args=(observed_time, observed_concentration),bounds = (lower_bounds, upper_bounds))
        J = result.jac
        parameter_names = ["CL", "Q", "V1", "V2"]
        residual_squared = (np.sum(result.fun**2))
        number_of_observations = len(observed_concentration)
        number_of_fparameters = len(result.x)
        JTJ = (J.T)@J
        JTJ_inverse = np.linalg.inv(JTJ)
        s_squared = (residual_squared)/(number_of_observations - number_of_fparameters)
        covariance_matrix = s_squared * JTJ_inverse
        variances = np.diag(covariance_matrix)
        standard_error = np.sqrt(variances)
        lower_bound = result.x - standard_error * 1.96
        upper_bound = result.x + standard_error * 1.96
        def is_suspicious(lower_bound, upper_bound):
            return (lower_bound <= 0) & (upper_bound >= 0)
        includes_zero = is_suspicious(lower_bound, upper_bound)
        for name, suspicious in zip(parameter_names, includes_zero):
            if suspicious:
                print(name, "Is suspicious")
        def test_suspicious():
            lower_bound_test = -2
            upper_bound_test = 5
            suspicious_test = is_suspicious(lower_bound_test, upper_bound_test)
            assert suspicious_test

        print(covariance_matrix)
        print(standard_error)

        fitted_parameters.append([
            subject,
            result.x[0],
            result.x[1],
            result.x[2],
            result.x[3],


        ])

    results_df = pd.DataFrame(
        fitted_parameters,
        columns=["Subject", "CL", "Q", "V1", "V2"]
    )


    return results_df
fit_model(df)