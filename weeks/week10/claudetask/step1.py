...
#changed result
perturbed_params = default_params._replace(CL = default_params.CL *1.10)
perturbed_result = run_pipeline(100.0, perturbed_params)

baseline_auc = baseline_result["auc_0_48"]
perturbed_auc = perturbed_result["auc_0_48"]

auc_change = perturbed_auc - baseline_auc

percent_change = (auc_change / baseline_auc) * 100
print("\nLocal Sensitivity Check:")
print(f"Baseline CL: {default_params.CL:.2f} L/h")
print(f"Perturbed CL (+10%): {perturbed_params.CL:.2f} L/h")
print(f"Baseline AUC_0_48: {baseline_auc:.2f}")
print(f"Perturbed AUC_0_48: {perturbed_auc:.2f}")
print(f"Change in AUC_0_48: {auc_change:.2f}")
print(f"Percent change in AUC_0_48: {percent_change:.2f}%")
